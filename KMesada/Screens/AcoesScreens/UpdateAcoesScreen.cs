using KMesada.Models;
using KMesada.Repositories;
using KMesada.Screens.AcoesScreens;

namespace KMesada.Screens;

public class UpdateAcoesScreen
{
    public static int Load()
    {
        int id = 0;
        bool check = true;
        var sair = "sim";
        //interessante imprimir a lista com o Id antes
        do
        {
            Console.Clear();
            Console.WriteLine("Atualização de Acoes das Crianças");
            Console.WriteLine("---------------------------");
            Console.WriteLine("Deseja listar as Ações para saber o Id? (S/N)");
            var resposta = Console.ReadLine();
            if (resposta!.Equals("S", StringComparison.OrdinalIgnoreCase))
            {
                ListAcoesScreen.List();
                
            }
                
            Console.WriteLine("Informe o ID Ação");
            id = Tratamentos.EntradaInt();
            check = ListAcoesScreen.consultaId(id);
            if (!check)
            {
                Console.WriteLine($"Usuário de ID {id} inexistente!");
                Console.ReadKey();
                Console.WriteLine("para voltar ao Menu digite (menu) \nOu Digite qualquer tecla para continuar");
                sair = Console.ReadLine();
                if (sair!.Equals("menu", StringComparison.OrdinalIgnoreCase) )
                {
                    Console.ReadKey();
                    MenuAcoesScreen.Load();
                    return 0;
                }
                    
            }
            
        } while (check is false);

        
        Console.WriteLine("Nome: ");
        var nome = Console.ReadLine();

        Console.WriteLine("Valor: ");
        var valor = int.Parse(Console.ReadLine()!);


        Update(new Acoes
        {
            Id = id,
            Nome = nome!,
            Valor = valor
        });

        Console.ReadKey();
        MenuAcoesScreen.Load();
        return 1;       

    }

    public static void Update(Acoes acoes)
    {
        try
        {
            var repository = new Repository<Acoes>();
            repository.Update(acoes);
            Console.WriteLine("Informações sobre Acoes das crianças atualizado com sucesso");
        }
        catch (Exception ex)
        {
            Console.WriteLine("Não foi possível atualizar os dados das ações das crianças");
            Console.WriteLine(ex.Message);
            Console.ReadKey();
            Load();
        }
    }
}